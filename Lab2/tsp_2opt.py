...
def two_opt(tour: List[int], matrix: List[List[int]]) -> List[int]:
    best = tour[:]
    improved = True
    best_distance = tour_length(best, matrix)
    n = len(best)
    ...
    while improved:
        ...
        for i in range(1, n - 1):
            for j in range(i + 1, n):
                ...
                new_distance = tour_length(new_tour, matrix)
...
def main():
    ...
    optimized_results = [
        ("nearestNeighbour + 2-opt", two_opt(nn_tour, matrix)),
        ("aex + 2-opt", two_opt(aex_tour, matrix)),
        ("hgrex + 2-opt", two_opt(hgrex_tour, matrix)),
    ]

    for name, tour in optimized_results:
        print(f"\nAlgorytm: {name}")
